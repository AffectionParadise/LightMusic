...
**运行**：clone 后打开该项目，等待 Maven 初始化完成，运行 **net.doge.ui.MainFrame** 类

**打包**：在项目结构中创建工件，然后构建该工件，就会获得一个可执行的jar包

**编译**：使用第三方软件编译为可执行程序，例如 exe4j。当然，不同的操作系统所使用的软件不同，需要使用者自己去研究
