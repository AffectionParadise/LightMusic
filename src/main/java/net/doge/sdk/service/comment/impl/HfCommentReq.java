...
public class HfCommentReq {
    private static HfCommentReq instance;

    private HfCommentReq() {
    }

    public static HfCommentReq getInstance() {
        if (instance == null) instance = new HfCommentReq();
        return instance;
    }
    ...
    public CommonResult<NetCommentInfo> getComments(NetResource resource, int page, int limit) {
        ...
        for (int i = 0, len = comments.size(); i < len; i++) {
            ...
            commentInfo.setSource(NetResourceSource.HF);
            ...
            ci.setSource(NetResourceSource.HF);
            ci.setSub(true);
