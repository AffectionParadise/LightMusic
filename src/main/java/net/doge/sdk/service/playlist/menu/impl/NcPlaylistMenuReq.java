package net.doge.sdk.service.playlist.menu.impl;
...
public class NcPlaylistMenuReq {
    private static NcPlaylistMenuReq instance;

    private NcPlaylistMenuReq() {
    }

    public static NcPlaylistMenuReq getInstance() {
        if (instance == null) instance = new NcPlaylistMenuReq();
        return instance;
    }

    // 歌单相似歌单 API (网易云)
    private final String SIMILAR_PLAYLIST_NC_API = "https://music.163.com/playlist?id=%s";
    // 歌单收藏者 API (网易云)
    private final String PLAYLIST_SUBSCRIBERS_NC_API = "https://music.163.com/weapi/playlist/subscribers";
    ...
    public CommonResult<NetPlaylistInfo> getSimilarPlaylists(NetPlaylistInfo netPlaylistInfo) {
        ...
        String playlistInfoBody = SdkCommon.ncRequest(Method.GET, String.format(SIMILAR_PLAYLIST_NC_API, id), "{}", options)
                .executeAsStr();
    ...
    public CommonResult<NetUserInfo> getPlaylistSubscribers(NetPlaylistInfo playlistInfo, int page, int limit) {
        ...
        String userInfoBody = SdkCommon.ncRequest(Method.POST, PLAYLIST_SUBSCRIBERS_NC_API,
