    ...
    public void fillMusicInfo(NetMusicInfo musicInfo) {
        ...
        if (!musicInfo.hasAlbumImage()) {
            GlobalExecutors.imageExecutor.execute(() -> {
                ...
                BufferedImage albumImage = SdkUtil.getImageFromUrl(String.format(SdkCommon.SONG_IMG_QQ_API, album.getString("mid")));
                // 有的歌曲没有专辑，先找备份专辑图片，如果还没有就将歌手的图片作为封面
                if (albumImage == ImageConstants.DEFAULT_IMG)
                    albumImage = SdkUtil.getImageFromUrl(String.format(SdkCommon.SONG_IMG_QQ_API, album.getString("pmid")));
                if (albumImage == ImageConstants.DEFAULT_IMG)
                    albumImage = SdkUtil.getImageFromUrl(String.format(SdkCommon.ARTIST_IMG_QQ_API, SdkUtil.parseArtistId(trackInfo)));
